...
public class TreeMap<K extends Comparable<K>, V> extends AbstractSortedMap<K, V> {
	...
	public TreeMap() {
		...
		tree = makeTree(); // use polymorphism
	...
	protected Position<Entry<K, V>> treeMin(Position<Entry<K, V>> p) {
		while (tree.isInternal(tree.left(p))) {
			p = tree.left(p);
		}
		return p;
	}
	...
	protected Position<Entry<K, V>> treeMax(Position<Entry<K, V>> p) {
		while (tree.isInternal(tree.right(p))) {
			p = tree.right(p);
		}
		return p;
	}
	...
	public V put(K key, V value) throws IllegalArgumentException, IOException {
		...
		if (tree.isExternal(p)) {
			...
			p.setElement(new MapEntry<>(key, value));
			rebalanceAccess(p);
			return old;
		}
	...
	public Entry<K, V> ceilingEntry(K key) throws IllegalArgumentException {
		...
		while (!tree.isRoot(p)) {
			...
			if (p == tree.left(parent)) return parent.getElement();
			p = parent;
		}
		return null;
	}
	...
	public Entry<K, V> floorEntry(K key) throws IllegalArgumentException {
		...
		while (!tree.isRoot(p)) {
			Position<Entry<K, V>> parent = tree.parent(p);
			if (p == tree.right(parent)) return parent.getElement();
			p = parent;
		}
		return null;
	}
	...
	public Iterable<Entry<K, V>> entrySet() {
		...
		if (!isEmpty()) {
			inorderSubtree(tree.root(), buffer);
		}
		return buffer;
	}

	private void inorderSubtree(Position<Entry<K, V>> p, ArrayList<Entry<K, V>> buffer) {
		if (tree.isExternal(p)) return;
		inorderSubtree(tree.left(p), buffer);
		buffer.add(p.getElement());
		inorderSubtree(tree.right(p), buffer);
	}
